...
const issueData = [
  {
    icoId: 'ico-people', color: '#1b3fa8', title: '「交流」の課題', subtitle: '個人同士が出会い協働する“場”が設計不十分', desc: '在留外国人と日本人の間には、継続的な交流を妨げる3つの壁が存在。「イベントに行っても写真だけで終わる」「連絡先を交換しても関係が続かない」という状態を生んでいる。',
  ...
  {
    icoId: 'ico-doc', color: '#0e6b52', title: '「情報」の課題', subtitle: '情報の取得〜行動までの流れが不十分', desc: '在留外国人が日本で生活する上で必要な情報へのアクセスが、4つの段階で途切れている。',
  ...
  {
    icoId: 'ico-pillars', color: '#a82f38', title: '「環境」の課題', subtitle: '鎖国的コミュニティ拡大と対立の激化', desc: '在留外国人の急増に対して、受け入れ側の体制整備が追いついていない。その結果、外国人コミュニティの孤立化と既住民との摩擦が深刻化。',
...
const bizData = [
  { name: 'International Friendship Tour', color: '#1b3fa8', seg: '交流推進事業', segBg: 'rgba(27,63,168,.1)', desc: '日本人と在留外国人の二日間型交流イベント。行動経済学に基づき開催日を2日に分け、参加費500円の低価格で深い関係構築を実現。1日目は座談会「connec+aトーク」、2日目は日帰り観光。' },
  { name: 'Ordermade', color: '#55606b', seg: '交流推進事業', segBg: 'rgba(85,96,107,.1)', desc: '日本語学校向けにカスタマイズした交流プランを企画設計から実行まで一貫提供。OM Culture（文化）・OM Education（教育）・OM Sports（スポーツ）の3パッケージ。非営利のため低価格。' },
  { name: 'connec+a community', color: '#3c7dbf', seg: '交流推進事業', segBg: 'rgba(60,125,191,.1)', desc: 'イベント終了後も継続的な交流の場を提供。Discordベースのオンラインプラットフォームでチャット・オンラインイベント・サークル活動を助成し、"熱量の蒸発"を阻止。' },
  { name: 'connec+a hub', color: '#0e6b52', seg: '情報PF事業', segBg: 'rgba(14,107,82,.1)', desc: '生活情報からイベント情報まで掲載する包括的デジタルプラットフォーム。多層タグ横断検索、3層言語表示（原文・やさしい日本語・多言語）、AIエージェント伴走による申請ナビ機能を搭載。' },
  { name: 'Public Affairs', color: '#e0801e', seg: '公共コンサル事業', segBg: 'rgba(224,128,30,.14)', desc: '自治体向けコンサルティングサービス。Hub行動ログ×住民インサイトで地域課題マップを作成し、IF/CC/OM/hubと連動した小規模実証→学習→展開の循環を確立。' }
];
let activeBiz = -1;
function showBiz(i) {
  ...
  activeBiz = i;
  el.style.setProperty('--c', d.color); // 詳細パネル左端の罫線を選択中の事業色に合わせる
...
async function loadNews() {
  ...
  if (!GAS_URL || GAS_URL === 'YOUR_GAS_DEPLOY_URL') {
    list.innerHTML = '<p class="news-empty">お知らせはありません</p>';
    return;
  }
  try {
    const items = await fetch(GAS_URL + '?type=news').then(r => r.json());
    if (!items.length) { list.innerHTML = '<p class="news-empty">お知らせはありません</p>'; return; }
    ...
    list.innerHTML = '<p class="news-empty">お知らせを読み込めませんでした</p>';
  }
}
